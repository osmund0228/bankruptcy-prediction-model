...
if st.session_state.get('run'):
    ...
    # 1. 고유 키(.st-key-my_report_box)를 가진 컨테이너에만 하늘색 배경 주입 (다른 곳에 영향 X)
    st.markdown(
        """
        <style>
        .st-key-my_report_box {
    ...
    # 2. 공식 컨테이너에 고유 key를 부여하여 디자인을 입히고, 그 안에 마크다운 바로 출력
    with st.container(key="my_report_box"):
